module.exports = {
  execute: async(client, member, deafType, Voice, VoiceConfig, event) => {
		let config;
    ...
    if (!config.trackDeaf) {
      ...
      if (!config.trackallchannels) {
        if (config.channelID.includes(member.voice.channel.id)) {
          ...
          let data = {}
					data.user = user
					data.config = config
					event.emit('userVoiceDeaf', data, member, member.voice.channel, deafType)
					return user;
					} else return;
        }
      }
      if (config.trackallchannels) {
		...
		return event.emit('userVoiceDeaf', data, member, channel, deafType);
		}
  }
}